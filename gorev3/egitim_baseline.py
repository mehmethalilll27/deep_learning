"""
GOREV 3 - ADIM 2: BASELINE (Intel Image Classification)

Kaggle veri seti uzerinde ilk model. Skoru YUKSELTMEK sonraki adimin isi;
buradaki amac durust bir baslangic noktasi olusturmak.

Baseline'da bilerek YOK:
  - augmentation      (A1'de eklenecek)
  - BatchNorm         (A2'de eklenecek)
  - dropout           (A2'de eklenecek)
  - lr scheduler      (A2'de eklenecek)
  - pretrained agirlik (A3'te eklenecek)

Bu model buyuk olasilikla ezberleyecek: train accuracy test'ten belirgin
yuksek cikacak. Bu bir kusur degil, sonraki adimlarin neyi duzelttigini
gosterecek olan kanittir.

Veri bolumu (Gorev 2'den farkli):
  seg_train %80 -> train        model burada ogrenir
  seg_train %20 -> validation   iyilestirme kararlari BURAYA bakarak verilir
  seg_test      -> test         yalnizca final olcum, konfig basina 1 kez
"""

import csv
import os
import random
import time

import numpy as np
import torch
import torch.nn as nn
from torch.utils.data import DataLoader, Subset
from torchvision import datasets, transforms

# 1) YOLLAR
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
PROJE_DIR = os.path.dirname(BASE_DIR)
KAYITLAR_DIR = os.path.join(PROJE_DIR, "kayitlar")
SONUC_DIR = os.path.join(KAYITLAR_DIR, "gorev3")
CONFUSION_DIR = os.path.join(SONUC_DIR, "confusion")
DATA_DIR = os.path.join(KAYITLAR_DIR, "data", "intel")

TRAIN_DIR = os.path.join(DATA_DIR, "seg_train", "seg_train")
TEST_DIR = os.path.join(DATA_DIR, "seg_test", "seg_test")

os.makedirs(SONUC_DIR, exist_ok=True)
os.makedirs(CONFUSION_DIR, exist_ok=True)


# 2) AYARLAR
SEED = 27
EPOCHS = 15
BATCH_SIZE = 64
LEARNING_RATE = 0.001
IMG_SIZE = 128         # orijinal 150x150 -> 128x128 (tum deneylerde sabit)
VAL_ORAN = 0.20        # seg_train'in %20'si validation
NUM_WORKERS = 4        # JPEG cozme CPU-bound, MNIST'ten farkli olarak gerekli
CIHAZ = "GPU"

DENEY_ADI = "baseline_cnn"

# ImageNet istatistikleri - A3'teki pretrained model bunlarla egitildi,
# bastan ayni degerleri kullanirsak transfer learning verimli olur.
NORM_ORTALAMA = [0.485, 0.456, 0.406]
NORM_STD = [0.229, 0.224, 0.225]

SONUC_DOSYASI = os.path.join(SONUC_DIR, "sonuclar_gorev3.csv")
SINIF_DOSYASI = os.path.join(SONUC_DIR, "sinif_bazli_gorev3.csv")


# 3) CIHAZ
def get_device():
    if not torch.cuda.is_available():
        raise RuntimeError("GPU bulunamadi! CUDA destekli PyTorch gerekli.")
    torch.backends.cudnn.benchmark = True
    return torch.device("cuda")


def seed_ayarla(seed=SEED):
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)


# 4) MODEL
class BasitCNN(nn.Module):
    """3 blok conv+pool, sonra 2 katmanli siniflandirici.

    BatchNorm ve dropout bilerek yok - baseline'in ezberlemesi bekleniyor.
    """

    def __init__(self, sinif_sayisi=6, img_size=IMG_SIZE):
        super().__init__()
        self.ozellik = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2),                     # 128 -> 64

            nn.Conv2d(32, 64, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2),                     # 64 -> 32

            nn.Conv2d(64, 128, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2),                     # 32 -> 16
        )
        # 3 kez yariya inildi: img_size / 8
        kenar = img_size // 8
        self.siniflandirici = nn.Sequential(
            nn.Flatten(),
            nn.Linear(128 * kenar * kenar, 256),
            nn.ReLU(),
            nn.Linear(256, sinif_sayisi),
        )

    def forward(self, x):
        return self.siniflandirici(self.ozellik(x))


def model_kur(sinif_sayisi, device):
    seed_ayarla(SEED)
    return BasitCNN(sinif_sayisi).to(device)


# 5) VERI
def transform_olustur(augment=False):
    """augment baseline'da False; A1'de True olacak.

    Resize'a (IMG_SIZE, IMG_SIZE) ikilisi verilmesi ONEMLI. Tek sayi verilirse
    en-boy orani korunur ve goruntuler farkli boyutta cikar; veri setindeki
    150x113 gibi birkac kare yuzunden batch olusturma hata verir.
    """
    adimlar = [transforms.Resize((IMG_SIZE, IMG_SIZE))]
    if augment:
        adimlar += [
            transforms.RandomHorizontalFlip(),
            transforms.RandomRotation(15),
        ]
    adimlar += [
        transforms.ToTensor(),
        transforms.Normalize(NORM_ORTALAMA, NORM_STD),
    ]
    return transforms.Compose(adimlar)


def katmanli_bol(dataset, val_oran=VAL_ORAN, seed=SEED):
    """Her siniftan ayni oranda ayirir - siniflar dengeli kalsin diye.

    ImageFolder'da .targets bir tensor degil duz liste, bu yuzden Gorev 2'deki
    train_data.targets.numpy() burada calismaz.
    """
    etiketler = np.array(dataset.targets)
    rng = np.random.default_rng(seed)

    train_idx, val_idx = [], []
    for sinif in np.unique(etiketler):
        idx = np.where(etiketler == sinif)[0]
        rng.shuffle(idx)
        kesme = int(round(len(idx) * val_oran))
        val_idx.extend(idx[:kesme].tolist())
        train_idx.extend(idx[kesme:].tolist())

    rng.shuffle(train_idx)
    rng.shuffle(val_idx)
    return train_idx, val_idx


def veri_yukle(augment=False):
    """Ayni klasoru iki ayri ImageFolder ile acar.

    Sebep: train'e augmentation uygulanmali ama validation'a UYGULANMAMALI.
    Tek dataset'i Subset ile bolsek ikisi de ayni transform'u paylasirdi.
    """
    egitim_tf = transform_olustur(augment=augment)
    olcum_tf = transform_olustur(augment=False)

    tam_egitim = datasets.ImageFolder(TRAIN_DIR, transform=egitim_tf)
    tam_olcum = datasets.ImageFolder(TRAIN_DIR, transform=olcum_tf)
    test_data = datasets.ImageFolder(TEST_DIR, transform=olcum_tf)

    train_idx, val_idx = katmanli_bol(tam_olcum)

    train_set = Subset(tam_egitim, train_idx)      # augmentation'li
    val_set = Subset(tam_olcum, val_idx)           # augmentation'siz
    train_olcum_set = Subset(tam_olcum, train_idx)  # overfit olcumu icin

    return train_set, val_set, test_data, train_olcum_set, tam_olcum.classes


def loader_hazirla(train_set, val_set, test_data, train_olcum_set):
    ortak = {"num_workers": NUM_WORKERS, "pin_memory": True,
             "persistent_workers": NUM_WORKERS > 0}
    train_loader = DataLoader(train_set, batch_size=BATCH_SIZE, shuffle=True, **ortak)
    val_loader = DataLoader(val_set, batch_size=128, shuffle=False, **ortak)
    test_loader = DataLoader(test_data, batch_size=128, shuffle=False, **ortak)
    train_olcum_loader = DataLoader(train_olcum_set, batch_size=128, shuffle=False, **ortak)
    return train_loader, val_loader, test_loader, train_olcum_loader


# 6) METRIKLER
def metricler_hesapla(model, loader, criterion, device, sinif_sayisi=6, eps=1e-12):
    """Tek geciste confusion matrix cikarir, metrikleri ondan turetir."""
    model.eval()
    toplam_loss = 0.0
    confusion = np.zeros((sinif_sayisi, sinif_sayisi), dtype=np.int64)

    with torch.no_grad():
        for images, labels in loader:
            images, labels = images.to(device), labels.to(device)
            logits = model(images)
            toplam_loss += criterion(logits, labels).item()

            preds = logits.argmax(dim=1)
            np.add.at(
                confusion,
                (labels.detach().cpu().numpy(), preds.detach().cpu().numpy()),
                1,
            )

    model.train()

    tp = np.diag(confusion).astype(np.float64)
    fp = confusion.sum(axis=0).astype(np.float64) - tp
    fn = confusion.sum(axis=1).astype(np.float64) - tp
    destek = confusion.sum(axis=1).astype(np.int64)

    precision = tp / (tp + fp + eps)
    recall = tp / (tp + fn + eps)
    f1 = 2.0 * precision * recall / (precision + recall + eps)

    return {
        "accuracy": tp.sum() / confusion.sum(),
        "loss": toplam_loss / len(loader),
        "precision_macro": precision.mean(),
        "recall_macro": recall.mean(),
        "f1_macro": f1.mean(),
        "precision": precision,
        "recall": recall,
        "f1": f1,
        "destek": destek,
        "confusion": confusion,
    }


def confusion_kaydet(confusion, deney_adi, siniflar):
    dosya = os.path.join(CONFUSION_DIR, f"{deney_adi}.csv")
    with open(dosya, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["gercek\\tahmin"] + list(siniflar))
        for i, satir in enumerate(confusion):
            writer.writerow([siniflar[i]] + satir.tolist())
    return dosya


def matris_yazdir(confusion, siniflar):
    """Satir = gercek, sutun = tahmin."""
    print("\n  CONFUSION MATRIX (test)")
    print("            " + "".join(f"{s[:7]:>9}" for s in siniflar) + "    <- TAHMIN")
    print("           +" + "-" * (9 * len(siniflar)))
    for i, satir in enumerate(confusion):
        hucreler = []
        for j, deger in enumerate(satir):
            if i == j:
                hucreler.append(f"{deger:>8} ")
            elif deger >= 10:
                hucreler.append(f"{deger:>8}*")
            elif deger > 0:
                hucreler.append(f"{deger:>8} ")
            else:
                hucreler.append(f"{'.':>8} ")
        print(f"  {siniflar[i][:9]:<9}|" + "".join(hucreler))
    print("  ^ GERCEK       ( * = 10+ yanlis tahmin,  . = 0 )")


def sinif_tablosu_yazdir(m, siniflar):
    print(f"\n  {'Sinif':<12}{'Destek':>8}{'Precision':>12}{'Recall':>10}{'F1':>10}")
    print("  " + "-" * 52)
    for s, ad in enumerate(siniflar):
        print(
            f"  {ad:<12}{m['destek'][s]:>8}"
            f"{m['precision'][s] * 100:>11.2f}%{m['recall'][s] * 100:>9.2f}%"
            f"{m['f1'][s] * 100:>9.2f}%"
        )
    print("  " + "-" * 52)
    print(
        f"  {'MACRO':<12}{m['destek'].sum():>8}"
        f"{m['precision_macro'] * 100:>11.2f}%{m['recall_macro'] * 100:>9.2f}%"
        f"{m['f1_macro'] * 100:>9.2f}%"
    )


def karisan_ciftler_yazdir(confusion, siniflar, ilk=5):
    satir_toplam = confusion.sum(axis=1)
    ciftler = []
    for i in range(len(siniflar)):
        for j in range(len(siniflar)):
            if i != j and confusion[i, j] > 0:
                ciftler.append((i, j, int(confusion[i, j]),
                                100.0 * confusion[i, j] / max(satir_toplam[i], 1)))
    ciftler.sort(key=lambda x: x[2], reverse=True)

    print("\n  EN COK KARISAN CIFTLER")
    for gercek, tahmin, adet, oran in ciftler[:ilk]:
        print(f"    {siniflar[gercek]:>10} -> {siniflar[tahmin]:<10} "
              f"{adet:>4} ornek (%{oran:.1f})")


# 7) EGITIM
def bir_epoch_egit(model, loader, criterion, optimizer, device):
    toplam_loss = 0.0
    for images, labels in loader:
        images, labels = images.to(device), labels.to(device)

        optimizer.zero_grad()
        loss = criterion(model(images), labels)
        loss.backward()
        optimizer.step()

        toplam_loss += loss.item()
    return toplam_loss / len(loader)


def train_model(model, loaders, device, epochs=EPOCHS):
    train_loader, val_loader, _, train_olcum_loader = loaders
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)

    gecmis = []
    en_iyi_val = 0.0
    en_iyi_epoch = 0
    start = time.perf_counter()

    for epoch in range(1, epochs + 1):
        loss = bir_epoch_egit(model, train_loader, criterion, optimizer, device)

        tr = metricler_hesapla(model, train_olcum_loader, criterion, device)
        va = metricler_hesapla(model, val_loader, criterion, device)
        fark = (tr["accuracy"] - va["accuracy"]) * 100

        if va["accuracy"] > en_iyi_val:
            en_iyi_val = va["accuracy"]
            en_iyi_epoch = epoch

        gecmis.append({
            "epoch": epoch,
            "train_loss": round(loss, 4),
            "train_acc": round(tr["accuracy"] * 100, 2),
            "val_acc": round(va["accuracy"] * 100, 2),
            "val_f1": round(va["f1_macro"] * 100, 2),
            "overfit_farki": round(fark, 2),
        })

        print(
            f"  Epoch {epoch:>2}/{epochs} | Loss: {loss:.4f} | "
            f"Train Acc: {tr['accuracy'] * 100:.2f}% | "
            f"Val Acc: {va['accuracy'] * 100:.2f}% | "
            f"Val F1: {va['f1_macro'] * 100:.2f}% | "
            f"Fark: {fark:+.2f} puan"
        )

    sure = time.perf_counter() - start
    return gecmis, sure, en_iyi_val, en_iyi_epoch


# 8) CSV KAYIT
def sonuclari_kaydet(sonuclar, dosya=SONUC_DOSYASI):
    fieldnames = [
        "deney", "model", "augmentation", "optimizer", "scheduler", "pretrained",
        "seed", "epochs", "img_size", "batch", "lr", "parametre",
        "train_acc", "val_acc", "val_f1", "en_iyi_val_acc", "en_iyi_epoch",
        "test_acc", "test_f1", "test_precision", "test_recall",
        "overfit_farki", "test_loss", "sure_sn", "cihaz", "notlar",
    ]
    with open(dosya, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(sonuclar)


def sinif_bazli_kaydet(satirlar, dosya=SINIF_DOSYASI):
    fieldnames = ["deney", "sinif", "test_destek", "precision", "recall", "f1"]
    with open(dosya, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(satirlar)


def gecmis_kaydet(gecmis, deney_adi):
    dosya = os.path.join(SONUC_DIR, f"epoch_gecmisi_{deney_adi}.csv")
    with open(dosya, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(gecmis[0].keys()))
        writer.writeheader()
        writer.writerows(gecmis)
    return dosya


# 9) CALISTIR
def main():
    device = get_device()
    seed_ayarla(SEED)

    train_set, val_set, test_data, train_olcum_set, siniflar = veri_yukle(augment=False)
    loaders = loader_hazirla(train_set, val_set, test_data, train_olcum_set)
    test_loader = loaders[2]

    print("=" * 78)
    print(f"GOREV 3 - BASELINE | Deney: {DENEY_ADI}")
    print("=" * 78)
    print(f"Siniflar      : {', '.join(siniflar)}  ({len(siniflar)} sinif)")
    print(f"Train         : {len(train_set)} goruntu")
    print(f"Validation    : {len(val_set)} goruntu")
    print(f"Test          : {len(test_data)} goruntu")
    print(f"Girdi         : {IMG_SIZE}x{IMG_SIZE}x3 | Batch: {BATCH_SIZE} | "
          f"Epoch: {EPOCHS} | LR: {LEARNING_RATE}")
    print(f"Augmentation  : yok | BatchNorm: yok | Dropout: yok | Scheduler: yok")
    print(f"Cihaz         : {CIHAZ} ({torch.cuda.get_device_name(0)})")

    model = model_kur(len(siniflar), device)
    parametre = sum(p.numel() for p in model.parameters())
    print(f"Parametre     : {parametre:,}".replace(",", "."))
    print()

    gecmis, sure, en_iyi_val, en_iyi_epoch = train_model(model, loaders, device)

    # Test seti YALNIZCA burada, egitim bittikten sonra bir kez olculur.
    criterion = nn.CrossEntropyLoss()
    test_m = metricler_hesapla(model, test_loader, criterion, device, len(siniflar))
    son = gecmis[-1]

    sonuc = {
        "deney": DENEY_ADI,
        "model": "BasitCNN",
        "augmentation": "yok",
        "optimizer": "Adam",
        "scheduler": "yok",
        "pretrained": False,
        "seed": SEED,
        "epochs": EPOCHS,
        "img_size": IMG_SIZE,
        "batch": BATCH_SIZE,
        "lr": LEARNING_RATE,
        "parametre": parametre,
        "train_acc": son["train_acc"],
        "val_acc": son["val_acc"],
        "val_f1": son["val_f1"],
        "en_iyi_val_acc": round(en_iyi_val * 100, 2),
        "en_iyi_epoch": en_iyi_epoch,
        "test_acc": round(test_m["accuracy"] * 100, 2),
        "test_f1": round(test_m["f1_macro"] * 100, 2),
        "test_precision": round(test_m["precision_macro"] * 100, 2),
        "test_recall": round(test_m["recall_macro"] * 100, 2),
        "overfit_farki": son["overfit_farki"],
        "test_loss": round(test_m["loss"], 4),
        "sure_sn": round(sure, 2),
        "cihaz": CIHAZ,
        "notlar": "baseline - augmentation/BN/dropout/scheduler yok",
    }

    sinif_satirlari = [
        {
            "deney": DENEY_ADI,
            "sinif": ad,
            "test_destek": int(test_m["destek"][s]),
            "precision": round(test_m["precision"][s] * 100, 2),
            "recall": round(test_m["recall"][s] * 100, 2),
            "f1": round(test_m["f1"][s] * 100, 2),
        }
        for s, ad in enumerate(siniflar)
    ]

    sonuclari_kaydet([sonuc])
    sinif_bazli_kaydet(sinif_satirlari)
    confusion_kaydet(test_m["confusion"], DENEY_ADI, siniflar)
    gecmis_kaydet(gecmis, DENEY_ADI)

    print("\n" + "=" * 78)
    print("BASELINE SONUCU")
    print("=" * 78)
    print(f"  Train accuracy : {son['train_acc']:.2f}%")
    print(f"  Val accuracy   : {son['val_acc']:.2f}%   (en iyi: "
          f"{en_iyi_val * 100:.2f}% @ epoch {en_iyi_epoch})")
    print(f"  TEST accuracy  : {test_m['accuracy'] * 100:.2f}%   <- BASELINE SKORU")
    print(f"  Test F1 (macro): {test_m['f1_macro'] * 100:.2f}%")
    print(f"  Overfit farki  : {son['overfit_farki']:+.2f} puan (train - val)")
    print(f"  Sure           : {sure:.1f} sn ({sure / 60:.1f} dk)")

    matris_yazdir(test_m["confusion"], siniflar)
    sinif_tablosu_yazdir(test_m, siniflar)
    karisan_ciftler_yazdir(test_m["confusion"], siniflar)

    print(f"\n  Sonuclar    : {SONUC_DOSYASI}")
    print(f"  Sinif bazli : {SINIF_DOSYASI}")
    print(f"  Confusion   : {CONFUSION_DIR}")
    print("=" * 78)


if __name__ == "__main__":
    main()
